...
function db(): PDO { static $pdo; if($pdo)return $pdo; $host=envv('DB_HOST','127.0.0.1');$port=envv('DB_PORT','3306');$name=envv('DB_NAME','urbanfit');
$dsn="mysql:host=$host;port=$port;dbname=$name;charset=utf8mb4";$opt=[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC,PDO::ATTR_EMULATE_PREPARES=>false];
if(envv('DB_SSL','false')==='true'){if(defined('PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT'))$opt[PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT]=false; if(defined('PDO::MYSQL_ATTR_SSL_CA') && envv('DB_SSL_CA'))$opt[PDO::MYSQL_ATTR_SSL_CA]=envv('DB_SSL_CA');}
